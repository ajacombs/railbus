...
public class Pois implements ForwardingProfile.LayerPostProcesser {
  ...
  public void processOsm(SourceFeature sf, FeatureCollector features) {
    if ((sf.isPoint() || sf.canBePolygon()) && (sf.hasTag("aeroway", "aerodrome") ||
      ...
      if (sf.hasTag("aeroway", "aerodrome")) {
        ...
        // One would think University should be earlier, but there are lots of dinky
        // node only places
        // So if the university has a large area, it'll naturally improve it's zoom in
        // the next section...
        minZoom = 14;
      ...
      if (sf.hasTag("boundary", "national_park")) {
        if (!(sf.hasTag("operator", "United States Forest Service", "US Forest Service", "U.S. Forest Service",
            "USDA Forest Service", "United States Department of Agriculture", "US National Forest Service",
            "United State Forest Service", "U.S. National Forest Service") ||
            sf.hasTag("protection_title", "Conservation Area", "Conservation Park", "Environmental use",
                "Forest Reserve",
      ...
      if (sf.canBePolygon() && sf.hasTag("name") && sf.getString("name") != null) {
        ...
        // Roughly shared with the water label area zoom grading in physical points
        // layer
        //
        ...
        if (kind.equals("national_park")) {
          ...
          // Typically for "building" derived label placements for shops and other
          // businesses
          ...
          // Small but tall features should show up early as they have regional
          // prominance.
        ...
        var polyLabelPosition = features.pointOnSurface(this.name())
            ...
            .setAttr("kind", kind)
            // While other layers don't need min_zoom, POIs do for more predictable
            // client-side label collisions
            ...
            // Special airport only tag (to indicate if it's an airport with regular
            // commercial flights)
        ...
        // Even with the categorical zoom bucketing above, we end up with too dense a
        // point feature spread in downtown
        // areas, so cull the labels which wouldn't label at earlier zooms than the
        // max_zoom of 15
        ...
        // Some features should only be visible at very late zooms when they don't have
        // a name
        ...
        if (sf.hasTag("amenity", "clinic", "dentist", "doctors", "social_facility", "baby_hatch", "childcare",
            "car_sharing", "bureau_de_change", "emergency_phone", "karaoke", "karaoke_box", "money_transfer",
            "car_wash",
            ...
            sf.hasTag("shop", "scuba_diving", "atv", "motorcycle", "snowmobile", "art", "bakery", "beauty", "bookmaker",
                ...
                "lottery", "mobile_phone", "newsagent", "optician", "perfumery", "ship_chandler", "stationery",
                "tobacco",
                "travel_agency")
            ||
  ...
  public List<VectorTile.Feature> postProcess(int zoom, List<VectorTile.Feature> items) throws GeometryException {
    ...
    // TODO: (nvkelso 20230623) Consider adding a "rank" here for POIs, like for
    // Places

...
import nz.ajacombs.wellingtontransitbasemap.feature.FeatureId;
import nz.ajacombs.wellingtontransitbasemap.feature.NaturalEarthDb;
import nz.ajacombs.wellingtontransitbasemap.names.OsmNames;
...
public class Places implements ForwardingProfile.LayerPostProcesser {
  ...
  // Evaluates place layer sort ordering of inputs into an integer for the
  // sort-key field.
  static int getSortKey(double minZoom, int kindRank, int populationRank, long population, String name) {
    return SortKey
        ...
        // Order ASCENDING (smaller manually curated Natural Earth min_zoom win over
        // larger values, across kinds)
        .orderByInt((int) minZoom, 0, 15)
        // Order ASCENDING (smaller values win, countries then locality then
        // neighbourhood, breaks ties for same minZoom)
        .thenByInt(kindRank, 0, 6)
        // Order DESCENDING (larger values win, San Francisco rank 11 wins over Oakland
        // rank 10)
        .thenByInt(populationRank, 15, 0)
        // Order DESCENDING (larger values win, Millbrea 40k wins over San Bruno 20k,
        // both rank 7)
        .thenByLog(population, 1000000000, 1, 100)
        // Order ASCENDING (shorter strings are better than longer strings for map
        // display and adds predictability)
  ...
  private static final ZoomFunction<Number> LOCALITY_GRID_SIZE_ZOOM_FUNCTION = ZoomFunction
      .fromMaxZoomThresholds(Map.of(
          6, 32,
          7, 64), 0);

  private static final ZoomFunction<Number> LOCALITY_GRID_LIMIT_ZOOM_FUNCTION = ZoomFunction
      .fromMaxZoomThresholds(Map.of(
          6, 8,
          7, 6,
          9, 4), 0);
    ...
    if (sf.isPoint() && sf.hasTag("name") &&
      ...
      switch (place) {
        ...
        case "farm":
          ...
          break;
        // NOTE (nvkelso 20240617): areas outside of main population center with
        // different postal city in Eastern Europe
        case "allotments":
          kind = "locality";
          // This minZoom can be changed to smaller value in the NE data join step below
          minZoom = 13.0f;
          ...
          break;
        // NOTE (nvkelso 20240617): suburb can mean locality in some countries and
        // should be localized
        case "suburb":
          kind = "neighbourhood";
          minZoom = 11.0f;
      ...
      if (kind.equals("locality")) {
        // We could add more fallback equivalency tests here, but 98% of NE places have
        // a Wikidata ID
      ...
      // Instead of exporting ISO country_code_iso3166_1_alpha_2 (which are sparse),
      // we export Wikidata IDs
      if (sf.hasTag("wikidata")) {
        feat.setAttr("wikidata", sf.getString("wikidata"));
      }

      // feat.setSortKey(minZoom * 1000 + 400 - populationRank * 200 +
      // placeNumber.incrementAndGet());
      feat.setSortKey(getSortKey(minZoom, kindRank, populationRank, population, sf.getString("name")));

      // We set the sort keys so the label grid can be sorted predictably (bonus: tile
      // features also sorted)
      // NOTE: The buffer needs to be consistent with the innteral grid pixel sizes
      // feat.setPointLabelGridSizeAndLimit(13, 64, 4); // each cell in the 4x4 grid
      // can have 4 items
